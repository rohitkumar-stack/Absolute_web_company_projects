...
import { DELETE_USER, GET_ALL_USERS, GET_DEPARTMENT, GET_DESIGNATION, GET_ITEM_CATEGORY, GET_TERMS_CONDITIONS, UPDATE_DEPARTMENT_STATUS, UPDATE_DESIGNATION_STATUS, UPDATE_ITEM_CATEGORY_STATUS, UPDATE_TERMS_CONDITIONS_STATUS, UPDATE_USER_STATUS, getToken } from '../../../globals';
...
const GetItemCategory = () => {
    const [breadcrumbItems] = useState([
        ...
        { title: 'Item Category ', link: process.env.PUBLIC_URL + '/itemcategorylist' },
    ]);
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Item_category Master"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Item_category Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete Item_category Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_ITEM_CATEGORY}?page=${page}` : `${GET_ITEM_CATEGORY}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete Item Category", {
                    type: "error",
                });
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_ITEM_CATEGORY_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    toast("Item Category Updated Successfully !", {
                        type: "success",
                    });
                    getAllUsers();
                    setShow(false);
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Item Category Status Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
            ...
            .catch((err) => {
                ...
                toast("Unable to Update Item Category", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
        ...
        return (
            <>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.category_description}</span>
                    </p>
                </td>
                <td>
                    <div
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Item Category list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Item Category"
                                addData={CreateButton == "Active" ? "/createitemcategory" : ""}
                                editData={EditButton == "Active" ? "/edititemcategory/" : ""}
            ...
            <Modal isOpen={show} backdrop="static">
                ...
                <ModalBody>
                    <p>Are you sure you want to delete this Item Category?</p>
                </ModalBody>
                <ModalFooter>
