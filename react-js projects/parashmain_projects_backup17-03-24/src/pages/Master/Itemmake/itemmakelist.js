...
import { DELETE_USER, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, UPDATE_ITEM_MAKE_STATUS, UPDATE_ITEM_SUB_CATGEORY_STATUS, } from '../../../globals';
import TableComponent from '../../../components/Common/tablecomponent';
import { ThreeDots } from "react-loader-spinner";
import { toast } from 'react-toastify';

const GetItemmakeList = () => {
    const [breadcrumbItems] = useState([
        { title: 'Master', link: process.env.PUBLIC_URL + '/dashboard' },
        { title: 'Item Make', link: process.env.PUBLIC_URL + '/itemmakelist' },
    ]);
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_ITEM_MAKER}?page=${page}` : `${GET_ITEM_MAKER}`, {
                method: 'GET',
                headers: {
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_ITEM_MAKE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Item Make Status Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
        ...
        return (
            <>
                ...
                <td>
                    ...
                    <p className="mb-0">
                        <span className="fw-medium">{item.make_name}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.contact_name}</span>
                    </p>
                </td>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.note}</span>
                    </p>
                </td>
                <td>
                    <div
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Item Make list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Item Make"
                                addData={CreateButton == "Active" ? "/createitemmake" : ""}
                                editData={EditButton == "Active" ? "/edititemmake/" : ""}
            ...
            <Modal isOpen={show} backdrop="static">
                ...
                <ModalBody>
                    <p>Are you sure you want to delete this Item Item Make?</p>
                </ModalBody>
                <ModalFooter>
...
export default GetItemmakeList;