...
import { DELETE_USER, GET_ALL_USERS, GET_DEPARTMENT, GET_DESIGNATION, GET_TERMS_CONDITIONS, UPDATE_DEPARTMENT_STATUS, UPDATE_DESIGNATION_STATUS, UPDATE_TERMS_CONDITIONS_STATUS, UPDATE_USER_STATUS, getToken } from '../../../globals';
...
const GetDesignation = () => {
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_DESIGNATION}?page=${page}` : `${GET_DESIGNATION}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete Designation", {
                    type: "error",
                });
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_DESIGNATION_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Designation Status Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
        ...
        return (
            <>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.designation_description}</span>
                    </p>
                </td>
                <td>
                    <div
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <Breadcrumbs title="Designation list" breadcrumbItems={breadcrumbItems} />
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                addData={CreateButton == "Active" ? "/createdesignation" : ""}
                                editData={EditButton == "Active" ? "/editdesignation/" : ""}
