...
import { DELETE_LEAVE, DELETE_USER, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_LEAD_REFERENCE, GET_LEAVE, GET_OWNERSHIP, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, GET_WAREHOUSE, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_LEAD_REFERENCE_STATUS, UPDATE_LEAVE_STATUS, UPDATE_OWNERSHIP_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, UPDATE_WAREHOUSE_STATUS, } from '../../../globals';
...
const GetownerShip = () => {
    const [breadcrumbItems] = useState([
        ...
        { title: 'Ownership', link: process.env.PUBLIC_URL + '/ownershiplist' },
    ]);
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_OWNERSHIP}?page=${page}` : `${GET_OWNERSHIP}`, {
                method: 'GET',
                headers: {
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_OWNERSHIP_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    toast("Ownership Updated Successfully !", {
                        type: "success",
                    });
                    getAllUsers();
                    setShow(false);
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Ownership Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
            ...
            .catch((err) => {
                ...
                toast("Unable to Update Ownership", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
    ...
    const AddOwnerShip = filteredPermissions?.find(permission => permission.name === "Add Ownership Master");
    const EditOwnerShip = filteredPermissions?.find(permission => permission.name === "Edit Ownership Master");
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Ownership list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Ownership"
                                addData={AddOwnerShip?.status == "Active" ? "/Createownership" : ""}
                                editData={EditOwnerShip?.status == "Active" ? "/editownership/" : ""}
