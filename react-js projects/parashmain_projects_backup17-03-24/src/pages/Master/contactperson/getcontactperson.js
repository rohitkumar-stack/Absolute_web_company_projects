...
import { DELETE_LEAVE, DELETE_USER, GET_CONTACTPERSON, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_LEAD_REFERENCE, GET_LEAVE, GET_OWNERSHIP, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, GET_WAREHOUSE, UPDATE_CONTACTPERSON_STATUS, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_LEAD_REFERENCE_STATUS, UPDATE_LEAVE_STATUS, UPDATE_OWNERSHIP_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, UPDATE_WAREHOUSE_STATUS, } from '../../../globals';
import TableComponent from '../../../components/Common/tablecomponent';
import { ThreeDots } from "react-loader-spinner";
import { toast } from 'react-toastify';

const GetContacPerson = () => {
    const [breadcrumbItems] = useState([
        { title: 'Master', link: process.env.PUBLIC_URL + '/dashboard' },
        { title: 'Contact Person', link: process.env.PUBLIC_URL + '/contactpersonlist' },
    ]);
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_CONTACTPERSON}?page=${page}` : `${GET_CONTACTPERSON}`, {
                method: 'GET',
                headers: {
        ...
        return (
            <>
                <td>
                    ...
                    <p className="mb-0">
                        <span className="fw-medium">{item.customer_type}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.mobile_no_1},<br />{item.mobile_no_2}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
   ...
   // Start------------RolePermission for Button 
    ...
    const AddContactPerson = filteredPermissions?.find(permission => permission.name === "Add Contact_person Master");
    const EditContactPerson = filteredPermissions?.find(permission => permission.name === "Edit Contact_person Master");
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Contact Person list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                addData={AddContactPerson?.status == "Active" ? "/createcontactperson" : ""}
                                editData={EditContactPerson?.status == "Active" ? "/editcontactperson/" : ""}
...
export default GetContacPerson;