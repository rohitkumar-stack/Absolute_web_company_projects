...
import { DELETE_USER, GET_ALL_USERS, GET_DEPARTMENT, GET_DESIGNATION, GET_ITEM_CATEGORY, GET_ITEM_SUB_CATEGORY, GET_TERMS_CONDITIONS, UPDATE_DEPARTMENT_STATUS, UPDATE_DESIGNATION_STATUS, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_TERMS_CONDITIONS_STATUS, UPDATE_USER_STATUS, getToken } from '../../../globals';
...
const GetItemSubCategory = () => {
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Item_sub_category Master"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Item_sub_category Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete Item_sub_category Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_ITEM_SUB_CATEGORY}?page=${page}` : `${GET_ITEM_SUB_CATEGORY}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .then((res) => {
                toast("Item Sub-Category Deleted !", {
                    type: "success",
                });
            ...
            .catch((err) => {
                toast("Unable to Delete Item Sub-Category", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_ITEM_SUB_CATGEORY_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Item Sub-Category Status Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
        ...
        return (
            <>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.sub_category_name}</span>
                    </p>
                </td>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.sub_category_description}</span>
                    </p>
                </td>
                <td>
                    <div
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Item Sub-Category list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Item Sub-Category"
                                addData={CreateButton == "Active" ? "/createitemsubcategory" : ""}
                                editData={EditButton == "Active" ? "/edititemsubcategory/" : ""}
            ...
            <Modal isOpen={show} backdrop="static">
                ...
                <ModalBody>
                    <p>Are you sure you want to delete this Item Sub-Category?</p>
                </ModalBody>
                <ModalFooter>
