...
import { DELETE_LEAVE, DELETE_USER, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_LEAD_REFERENCE, GET_LEAVE, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, GET_WAREHOUSE, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_LEAD_REFERENCE_STATUS, UPDATE_LEAVE_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, UPDATE_WAREHOUSE_STATUS, } from '../../../globals';
import TableComponent from '../../../components/Common/tablecomponent';
import { ThreeDots } from "react-loader-spinner";
import { toast } from 'react-toastify';

const GetLeadreference = () => {
    const [breadcrumbItems] = useState([
        { title: 'Master', link: process.env.PUBLIC_URL + '/dashboard' },
        { title: 'Lead Reference ', link: process.env.PUBLIC_URL + '/leadreferencelist' },
    ]);
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_LEAD_REFERENCE}?page=${page}` : `${GET_LEAD_REFERENCE}`, {
                method: 'GET',
                headers: {
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_LEAD_REFERENCE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Lead Reference Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
    ...
    const  EditLeadReference = filteredPermissions?.find(permission => permission.name === "Edit Reference Master");
    // console.log(EditLeadReference, "====>EditLeadReference");
    //  console.log(leadRafActive, "====>leadRafActive");
    ...
    return (
        ...
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Lead Reference list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Lead Reference"
                                addData={leadRafActive?.status == "Active" ? "/Createleadreference" : ""}
                                editData={EditLeadReference?.status == "Active" ? "/editleadreference/" : ""}
...
export default GetLeadreference;