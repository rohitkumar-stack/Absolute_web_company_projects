...
import {
  ...
  DELETE_USER,
  GET_CONTACTPERSON,
  GET_FINDDEPARTMENTCOMPANYWISE,
  GET_ALLDEPARTMENTCOMPANYWISE,
...
const AddDepartment = () => {
  ...
  console.log("checkselectedata", departmentCompanyWise);
  console.log("checkselectedata22", allDepartment);
  const PermissionArr =
  ...
  const getAllDepartment = async (page, perPage) => {
    setisLoading(true);
    var Token = await localStorage.getItem("userToken");
    try {
      const response = await fetch(
        page
          ? `${GET_ALLDEPARTMENTCOMPANYWISE}?page=${page}`
          : `${GET_ALLDEPARTMENTCOMPANYWISE}`,
        {
          method: "GET",
          headers: {
  ...
  const getAllCompany = async (page, perPage) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.result === true) {
        console.log("checkDesignationData12", data);
  ...
  const findDepartmentCompanyWise = async (page, perPage) => {
    setisLoading(true);
    var Token = await localStorage.getItem("userToken");
    var id = 123;
    ...
    try {
      const response = await fetch(
        page
          ? `${GET_FINDDEPARTMENTCOMPANYWISE}?page=${page}${selectedCompany}`
          : `${GET_FINDDEPARTMENTCOMPANYWISE}${selectedCompany}`,
        {
          method: "GET",
          headers: {
            Authorization: "Bearer " + Token,
      ...
      const data = await response.json();
      if (data.result === true) {
        console.log("checkselecteddeprtment",data)
  ...
  console.log("checkdesignationpermision", AddDesignation?.status);
      ...
      <Container fluid>
        ...
        <CardBody>
          ...
          {isLoading ? (
            ...
            <TableComponent
              allUserData={departmentCompanyWise?departmentCompanyWise:allDepartment || []}
              perPage={perPage}
