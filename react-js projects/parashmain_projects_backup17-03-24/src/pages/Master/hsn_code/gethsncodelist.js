...
import { DELETE_COUNTRY, DELETE_HSN_CODE, DELETE_USER, GET_ALL_USERS, GET_COUNTRY, GET_HSN_CODE, UPDATE_HSN_CODE_STATUS, getToken } from '../../../globals';
...
const GetHSNCode = () => {
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Hsn_code Master"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Hsn_code Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete Hsn_code Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_HSN_CODE}?page=${page}` : `${GET_HSN_CODE}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete HSN Code", {
                    type: "error",
                });
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_HSN_CODE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("HSN Code Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
        ...
        return (
            <>
                ...
                {/* <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.taxation_value + " " + "₹"}</span>
                    </p>
                </td> */}
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <Breadcrumbs title="HSN Code list" breadcrumbItems={breadcrumbItems} />
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add HSN Code"
                                addData={CreateButton == "Active" ? "/createhsncode" : ""}
                                editData={EditButton == "Active" ? "/edithsncode/" : ""}
            ...
            <Modal isOpen={show} backdrop="static">
                ...
                <ModalBody>
                    <p>Are you sure you want to delete this HSN Code?</p>
                </ModalBody>
                <ModalFooter>
