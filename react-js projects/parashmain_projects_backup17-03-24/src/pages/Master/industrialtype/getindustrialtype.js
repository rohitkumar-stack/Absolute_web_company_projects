...
import { DELETE_LEAVE, DELETE_USER, GET_INDUSTRIAL_TYPE, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_LEAD_REFERENCE, GET_LEAVE, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, GET_WAREHOUSE, UPDATE_INDSUTRIAL_TYPE_STATUS, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_LEAD_REFERENCE_STATUS, UPDATE_LEAVE_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, UPDATE_WAREHOUSE_STATUS, } from '../../../globals';
import TableComponent from '../../../components/Common/tablecomponent';
import { ThreeDots } from "react-loader-spinner";
import { toast } from 'react-toastify';

const GetIndustrialType = () => {
    const [breadcrumbItems] = useState([
        { title: 'Master', link: process.env.PUBLIC_URL + '/dashboard' },
        { title: 'Industrial Type', link: process.env.PUBLIC_URL + '/industrialtypelist' },
    ]);
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_INDUSTRIAL_TYPE}?page=${page}` : `${GET_INDUSTRIAL_TYPE}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete Industrial Type", {
                    type: "error",
                });
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_INDSUTRIAL_TYPE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    toast("Industrial Type Updated Successfully !", {
                        type: "success",
                    });
                    getAllUsers();
                    setShow(false);
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Industrial Type Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
            ...
            .catch((err) => {
                ...
                toast("Unable to Update Industrial Type", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
    ...
    const AddIndustrialType = filteredPermissions?.find(permission => permission.name === "Add Industrial_type Master");
    const EditIndustrialType = filteredPermissions?.find(permission => permission.name === "Edit Industrial_type Master");
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Industrial Type list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Industrial Type"
                                addData={AddIndustrialType?.status == "Active" ? "/Createindustrialtype" : ""}
                                editData={EditIndustrialType?.status == "Active" ? "/editindustrialtype/" : ""}
...
export default GetIndustrialType;