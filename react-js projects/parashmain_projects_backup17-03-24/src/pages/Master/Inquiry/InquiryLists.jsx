...
import { DELETE_LEAVE, DELETE_USER, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_LEAD_REFERENCE, GET_LEAVE, GET_INQUIRY, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, GET_WAREHOUSE, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_LEAD_MARKETING_STATUS, UPDATE_LEAD_REFERENCE_STATUS, UPDATE_LEAVE_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, UPDATE_WAREHOUSE_STATUS, } from '../../../globals';
...
const InquiryLists = () => {
    const [breadcrumbItems] = useState([
        ...
        { title: 'inquiry Lead', link: process.env.PUBLIC_URL + '/inquirylist' },
    ]);
    ...
    const GetAllInquiryData = async (page, perPage) => {
        setisLoading(true)
        var Token = await localStorage.getItem("userToken");
        try {
            const response = await fetch(page ? `${GET_INQUIRY}?page=${page}` : `${GET_INQUIRY}`, {
                method: 'GET',
                headers: {
            ...
            if (data.result === true) {
                ...
                // console.log(userlist, "==========>GetAllInquiryData")
                setisLoading(false)
   ...
   //DELETE INQUIRY LISTS
        ...
        return (
            <>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.lead_topic}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.order_probability}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.notes}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
    ...
    const AddInquiry = filteredPermissions?.find(permission => permission.name === "Add Inquiry");
    const EditInquiry = filteredPermissions?.find(permission => permission.name === "Edit Inquiry");
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <Breadcrumbs title="Inquiry Lead list" breadcrumbItems={breadcrumbItems} />
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                setShow={setShow}
                                GetAllInquiryData={GetAllInquiryData}
                                renderTHContent={renderTHContent}
                                renderTdContent={renderTdContent}
                                btnName="Add Inquiry Lead"
                                // addData={CreateButton == "Active" ? "/createmarketlead" : ""}
                                addData={AddInquiry?.status == "Active" ? "/createInquiry" : ""}
                                editData={EditInquiry?.status == "Active" ? "/editinquiry/" : ""}
