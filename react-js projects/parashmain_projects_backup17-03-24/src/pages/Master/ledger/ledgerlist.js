...
import { DELETE_COUNTRY, DELETE_HSN_CODE, DELETE_LEDGER_GROUP, DELETE_USER, GET_ALL_USERS, GET_COUNTRY, GET_HSN_CODE, GET_LEDGER, GET_LEDGER_GROUP, UPDATE_LEDGER_STATUS, getToken } from '../../../globals';
...
const LeagderlList = () => {
    const [breadcrumbItems] = useState([
        ...
        { title: 'Ledger', link: process.env.PUBLIC_URL + '/ledgerlist' },
    ]);
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Ledger Master"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Ledger Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete Ledger Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_LEDGER}?page=${page}` : `${GET_LEDGER}`, {
                method: 'GET',
                headers: {
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_LEDGER_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Ledger Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
        ...
        return (
            <>
                <td>
                    ...
                    <p className="mb-0">
                        <span className="fw-medium">{item.ledger_type}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.contact_person}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.phone_no}</span>
                    </p>
                </td>
                {/* <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.gstin}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.pan_no}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.limit_upto}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.address}</span>
                    </p>
                </td>
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                ...
                {/* <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.state_code}</span>
                    </p>
                </td> */}
                {/* <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.website}</span>
                    </p>
                </td> */}
                <td>
                    <div
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Ledger list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Ledger"
                                addData={CreateButton == "Active" ? "/addledger" : ""}
                                editData={EditButton == "Active" ? "/editledger/" : ""}
                                MasterSearch={'ledger_type'}
