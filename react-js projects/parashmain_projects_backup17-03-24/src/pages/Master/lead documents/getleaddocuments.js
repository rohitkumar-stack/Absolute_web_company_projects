...
import { DELETE_COUNTRY, DELETE_STATE, DELETE_USER, GET_ALL_USERS, GET_COUNTRY, GET_LEAD_DOCUMENT, GET_STATE, UPDATE_STATE_STATUS, getToken } from '../../../globals';
...
const GetLeadDocuments = () => {
    const [breadcrumbItems] = useState([
        ...
        { title: 'Lead Document', link: process.env.PUBLIC_URL + '/leaddocumentlist' },
    ]);
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add State Master"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit State Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete State Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_LEAD_DOCUMENT}?page=${page}` : `${GET_LEAD_DOCUMENT}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete Lead Document", {
                    type: "error",
                });
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_STATE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    toast("Lead Document Updated Successfully !", {
                        type: "success",
                    });
                    getAllUsers();
                    setShow(false);
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Lead Document Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
            ...
            .catch((err) => {
                ...
                toast("Unable to Update Lead Document", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
        ...
        return (
            <>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.document_type}</span>
                    </p>
                </td>
                {/* <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.country_id}</span>
                    </p>
                </td> */}
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.document_name}</span>
                    </p>
                </td>
                <td>
                    <div
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <Breadcrumbs title="Lead Document list" breadcrumbItems={breadcrumbItems} />
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Lead Document"
                                addData={CreateButton == "Active" ? "/createleaddocument" : ""}
                                editData={EditButton == "Active" ? "/editstate/" : ""}
                                deletedata={DeleteButton == "Active" ? "Active" : ""}
                                MasterSearch={'state_name'}
                            // emailSearch1={'email'}
                            />
                        )}
                    </CardBody>
            ...
            <Modal isOpen={show} backdrop="static">
                ...
                <ModalBody>
                    <p>Are you sure you want to delete Lead Document?</p>
                </ModalBody>
                <ModalFooter>
...
export default GetLeadDocuments;