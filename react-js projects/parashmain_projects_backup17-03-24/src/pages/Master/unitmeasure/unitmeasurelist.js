...
import { DELETE_USER, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, } from '../../../globals';
...
const GetUnitMeasure = () => {
    const [breadcrumbItems] = useState([
        ...
        { title: 'Unit Measure', link: process.env.PUBLIC_URL + '/unitmeasurelist' },
    ]);
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Unit Measure Master"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Unit Measure Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete Unit Measure Master"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_UNIT_MEASURE}?page=${page}` : `${GET_UNIT_MEASURE}`, {
                method: 'GET',
                headers: {
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_UNIT_MEASURE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    toast("Unit Measure Updated Successfully !", {
                        type: "success",
                    });
                    getAllUsers();
                    setShow(false);
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Unit Measure Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
            ...
            .catch((err) => {
                ...
                toast("Unable to Update Unit Measure", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
    ...
    return (
        <React.Fragment>
            ...
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Unit Measure list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Unit Measure"
                                addData={CreateButton == "Active" ? "/createunitmeasure" : ""}
                                editData={EditButton == "Active" ? "/editunitmeasure/" : ""}
            ...
            <Modal isOpen={show} backdrop="static">
                ...
                <ModalBody>
                    <p>Are you sure you want to delete this Unit Measure?</p>
                </ModalBody>
                <ModalFooter>
