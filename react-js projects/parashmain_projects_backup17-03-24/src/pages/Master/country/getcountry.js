...
import { DELETE_COUNTRY, GET_COUNTRY, UPDATE_COUNTRY_STATUS } from '../../../globals';
...
const GetCountry = () => {
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Country"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Country"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_COUNTRY}?page=${page}` : `${GET_COUNTRY}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete Country", {
                    type: "error",
                });
        ...
        await fetch(UPDATE_COUNTRY_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Country Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <Breadcrumbs title="Country list" breadcrumbItems={breadcrumbItems} />
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Country"
                                addData={CreateButton == "Active" ? "/createcountry" : ""}
                                editData={EditButton == "Active" ? "/editcountry/" : ""}
