...
import { DELETE_LEAVE_BALANCE, DELETE_USER, GET_ITEM_MAKER, GET_ITEM_SUB_CATEGORY, GET_LEAVE, GET_LEAVE_BALANCE, GET_SHIFT, GET_UNIT, GET_UNIT_MEASURE, GET_WAREHOUSE, UPDATE_ITEM_SUB_CATGEORY_STATUS, UPDATE_LEAVE_BALANCE_STATUS, UPDATE_LEAVE_STATUS, UPDATE_SHIFT_STATUS, UPDATE_UNIT_MEASURE_STATUS, UPDATE_UNIT_STATUS, UPDATE_WAREHOUSE_STATUS, } from '../../../globals';
...
const GetLeaveBalance = () => {
    ...
    const loadCreatePermissions = async () => {
        try {
            ...
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Add Leave Balance"))
    ...
    const loadEditPermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Edit Leave Balance"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const loadDeletePermissions = async () => {
        try {
            const tempArr = PermissionArr
                .filter(item => item.name.includes("Delete Leave Balance"))
                .flatMap(item => sidebarArr
                    .filter(sidebar => item.type === sidebar.value)
                    .map(sidebar => ({
                        active: item.status,
    ...
    const getAllUsers = async (page, perPage) => {
        ...
        try {
            const response = await fetch(page ? `${GET_LEAVE_BALANCE}?page=${page}` : `${GET_LEAVE_BALANCE}`, {
                method: 'GET',
                headers: {
    ...
    const DeleteUserData = async (isUserID) => {
        ...
        await fetch(
            ...
            .catch((err) => {
                toast("Unable to Delete Leave Balance", {
                    type: "error",
                });
    ...
    const updateContactStatus = async (userid, status) => {
        setisLoading(true);
        var Token = localStorage.getItem("userToken");
        await fetch(UPDATE_LEAVE_BALANCE_STATUS + userid, {
            ...
            response.json().then((data) => {
                if (data.result == true) {
                    toast("Leave Balance Updated Successfully !", {
                        type: "success",
                    });
                    getAllUsers();
                    setShow(false);
                    ...
                    getAllUsers();
                    setisLoading(false);
                    toast("Leave Balance Cannot Be Updated", {
                        type: "error",
                    });
                }
            });
            ...
            .catch((err) => {
                ...
                toast("Unable to Update Leave Balance", {
                    type: "error",
                });
                setShow(false);
                setisLoading(false);
        ...
        return (
            ...
            {/* <td>
                ...
                <td>
                    <h5 className="font-size-14 text-truncate"></h5>
                    <p className="mb-0">
                        <span className="fw-medium">{item.balance}</span>
                    </p>
                </td>
    ...
    return (
        <React.Fragment>
            {/* <div className="page-content"> */}
                <Container fluid>
                    {/* <Breadcrumbs title="Leave Balance list" breadcrumbItems={breadcrumbItems} /> */}
                    ...
                    <CardBody>
                        {isLoading ? (
                            ...
                            <TableComponent
                                ...
                                btnName="Add Leave Balance"
                                addData={CreateButton == "iActive" ? "/Createleavebalance" : ""}
                                editData={EditButton == "iActive" ? "/editleavebalance/" : ""}
