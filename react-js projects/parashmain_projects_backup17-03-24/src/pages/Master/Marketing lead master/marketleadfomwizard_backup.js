...
import {
    GET_MARKETING_LEAD, FIND_CONTACTPERSON_DESINATION_BYID, GET_CONTACTPERSON_DEPARTMENT,
    CREATE_DOCUMENT_LEAD, GET_CONTACTPERSON_SEARCH, FIND_CONTACTPERSON_DEPARTMENT_BYID, CREATE_LEAD_REFERENCE, CREATE_LEAVE, CREATE_MARKETING_LEAD, CREATE_MARKETING_LEAD_WIDTH_DOCUMENTS, CREATE_SHIFT, CREATE_SUBITEM_CATEGORY, CREATE_TERMS_CONDITIONS, CREATE_UNIT, CREATE_UNIT_MEASURE, CREATE_USER, CREATE_WAREHOUSE, GET_ALL_CUSTOMER_WO_PAGINATION, GET_CONTACTPERSON_BY_ID, GET_CONTACTPERSON_WO_PAGINATE, GET_COUNTRY, GET_CUSTOMER_BY_ID, GET_ITEM_CATEGORY, GET_LEAD_REFERENCE_BY_ID, GET_LEAD_REFERENCE_WO_PAGINATE, GET_OWNERSHIP_WO_PAGINATE
} from "../../../globals";
import CustomFileInputNew from "../../../components/Common/imagefunction";
import Dropzone from "react-dropzone";
import { Link } from "react-router-dom/cjs/react-router-dom.min";
import { Row, Col, Card, CardBody, TabContent, CardHeader, TabPane, NavItem, NavLink, Label, Input, Form, Progress, Container, Button, Table, ModalBody, ModalHeader, Modal } from "reactstrap";
...
class MarketLeadForWizard extends Component {
    ...
    async FindAllContactsDesination(selectedCopmany) {
        ...
        var Token = localStorage.getItem("userToken");
        try {
            const response = await fetch(FIND_CONTACTPERSON_DESINATION_BYID + selectedCopmany, {
                method: "GET",
                headers: {
                    Authorization: "Bearer " + Token,
                    "Content-Type": "application/json",
    ...
    async FindAllContactsDepartment(selectedCompany) {
        ...
        var Token = localStorage.getItem("userToken");
        try {
            const response = await fetch(FIND_CONTACTPERSON_DEPARTMENT_BYID + selectedCompany, {
                method: "GET",
                headers: {
                    Authorization: "Bearer " + Token,
                    "Content-Type": "application/json",
    ...
    async getAllContactsPersonSearch() {
        ...
        const { selectedCompany, selectedDepartment, selectedDesination } = this.state;
    ...
    async StepOneSubmit(event, values) {
        await this.setState({
            ...
            reference_person_description: values.referenceName,
    ...
    async StepTwoSubmit(event, values) {
        const { dataSlectContactArray } = this.state;
        await this.setState({
            customer_id_encode: values.customerid,
            contactpersonDetails: dataSlectContactArray,
    ...
    handleFormSubmit = (e) => {
        e.preventDefault();
        const { selectedcontactpersonAddMore, ContactPersonDescriptionAddMore } = this.state;

        // Constructing the new row data
        const newRowData = {
            selectedcontactperson: ContactPersonDescriptionAddMore.first_name + " " + ContactPersonDescriptionAddMore.last_name,
        ...
        this.setState(prevState => ({
            ...
            ContactPersonDescriptionAddMore: { // Reset ContactPersonDescriptionAddMore object
                first_name: "",
                last_name: "",
                mobile_no_1: "",
                department_name: "",
    ...
    deleteAddMorwRow = (Index) => {
        this.setState(prevState => {
            ...
            const updatedDataArray = prevState.dataArray.filter((_, i) => i !== Index);
    ...
    handleFormContatPersonSubmit = (e) => {
        e.preventDefault();
        const { selectedcontactpersonAddMore, ContactPersonSearch, selectedCustomerServiceTypes } = this.state;

        // Constructing the new row data
        const newRowData = {
            contact_person_id_encode: ContactPersonSearch.id,
        };

        // Update the array of data in the state
        this.setState(prevState => ({
            dataSlectContactArray: [...(prevState.dataSlectContactArray || []), newRowData],
            ContactAddMore_Model: false,  // Closing the modal
            selectedcontactpersonAddMore: "", // Reset selected contact person
            ContactPersonSearch: { // Reset CompanyType object
                contact_person_id_encode: "",
            },
            selectedCustomerServiceTypes: [], // Reset selectedCustomerTypes array
        }), () => {
            console.log('dataSlectContactArray:', this.state.dataSlectContactArray);
    ...
    render() {
        const { isAadharBase64URL, fileData, formData, contactpersonlistAddMore,
            selectedcontactpersonAddMore, dataArray, FindContactDesignation, customerlist,
            Companydescription, FindContactDepartment, ContactPersonSearch, companyType, selectedCopmany } = this.state;
        // console.log(selectedCopmany, "=====>selectedCopmany");
                                        ...
                                        <TabContent activeTab={this.state.activeTab} className="twitter-bs-wizard-tab-content">
                                            <TabPane tabId={1}>
                                                ...
                                                >
                                                    <Row className="mt-2">
                                                        ...
                                                        <Col lg="3">
                                                            <Label className="form-label" htmlFor="validationCustom01"
                                                                style={{ fontSize: "14px" }}
                                                            >
                                                                Priority <span style={{ color: "#ff0000" }}>*</span>
                                                            </Label>
                                                            <AvField
                                                                name="priority"
                                                        ...
                                                        <Col lg="3" className="d-inline">
                                                            ...
                                                            <AvField
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    this.setState({ selectedlead: selectedValue });
                                                                    if (selectedValue !== "") {
                                                        ...
                                                        {this.state.selectedlead !== "" && (
                                                            <>
                                                                {/* Start---Offline  */}
                                                                {this.state.selectedlead === '76qxErGXQ2yXP3lV' ? (
                                                                    <Row>
                                                                        <Col lg={{ size: 3 }}>
                                                                            <Label
                                                                                className="form-label"
                                                                ...
                                                                ) : this.state.selectedlead === 'OaPvk1z45GN6Yd3M' ? (<><Col lg={{ size: 3 }}>
                                                                    <Label
                                                                        className="form-label"
                                                                        htmlFor="validationCustom01"
                                                                        style={{ fontSize: "14px" }}
                                                    ...
                                                    <div className="d-flex justify-content-between mt-5">
                                                        <Button color="primary" className={this.state.activeTab === 1 ? "previous disabled" : "previous"} >Previous</Button>
                                                ...
                                                <div>
                                                    ...
                                                    >
                                                                ...
                                                                <Row>
                                                                    <div className="company_radio_btn">
                                                                        ...
                                                                        <div className="form-check d-inline mx-2">
                                                                            ...
                                                                            />
                                                                            <label className="form-check-label" htmlFor="existingCompany">
                                                                                Existing Company
                                                                            </label>
                                                                        </div>
                                                                        <div className="form-check d-inline">
                                                                            <input
                                                                                ...
                                                                                onChange={this.handleChange}
                                                                            />
                                                                            <label className="form-check-label" htmlFor="newCompany">
                                                                                New Company
                                                                            </label>
                                                                        </div>
                                                                    </div>
                                                                            ...
                                                                            <Row>
                                                                                    ...
                                                                                    <AvField
                                                                                        ...
                                                                                        onChange={(e) => {
                                                                                            ...
                                                                                            this.setState({
                                                                                                ...
                                                                                                this.FindAllContactsDepartment(selectedCompany); // Call FindAllContactsDepartment with the selected company ID
                                                                                                this.FindAllContactsDesination(selectedCompany);
                                                                                                this.getAllContactsPersonSearch();
                                                                                            });
                                                                                        }}
                                                                                    >
                                                                                        ...
                                                                                        {Object.keys(this.state.customerlist).map((key) => (
                                                                                            <option key={key} value={this.state.customerlist[key].id}>
                                                                                                {this.state.customerlist[key].company_name}
                                                                                            </option>
                                                                                        ))}
                                                                                    </AvField>
                                                                                ...
                                                                                <Col lg="4" className="d-inline">
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        {Object.keys(this.state.FindContactDepartment).map((key) => (
                                                                                            <option key={key} value={this.state.FindContactDepartment[key].id}>
                                                                                                {this.state.FindContactDepartment[key].department_name}
                                                                                            </option>
                                                                                        ))}
                                                                                    </AvField>
                                                                                </Col>
                                                                                <Col lg="4" className="d-inline">
                                                                                    ...
                                                                                    <AvField
                                                                                        ...
                                                                                        onChange={(e) => {
                                                                                            ...
                                                                                            console.log('Selected Designation ID:', selectedDesination);
                                                                                            this.setState({
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        {/* <option value={""} >Other</option> */}
                                                                                        {Object.keys(this.state.FindContactDesignation).map((key) => (
                                                                                            <option key={key} value={this.state.FindContactDesignation[key].id}>
                                                                                                {this.state.FindContactDesignation[key].designation_name}
                                                                                            </option>
                                                                                        ))}
                                                                                    </AvField>
                                                                                </Col>
                                                                                ...
                                                                                {this.state.isLoading ? (
                                                                                    ...
                                                                                    <>
                                                                                        {ContactPersonSearch.length === 0 ? (
                                                                                            ...
                                                                                            <>
                                                                                                {this.state.ContactPersonSearch.map((item, index) => (
                                                                                                    <tr key={index}>
                                                                                                        <td>
                                                                                                            {/* <div className="form-check">
                                                                                                                <Input type="checkbox" className="form-check-input" id={`customControlInline_${index}`} />
                                                                                                            </div> */}
                                                                                                            <div className="form-check">
                                                                                                                <Input
                                                                                                                    type="checkbox"
                                                                                                                    ...
                                                                                                                    onChange={(e) => {
                                                                                                                        ...
                                                                                                                        if (checked) {
                                                                                                                            this.setState(prevState => ({
                                                                                                                                dataSlectContactArray: [
                                                                                                                                    ...prevState.dataSlectContactArray,
                                                                                                                                    { contact_person_id_encode: value }
                                                                                                                                ]
                                                                                                                            }), () => console.log("Selected values:", this.state.dataSlectContactArray));
                                                                                                                        } else {
                                                                                                                            this.setState(prevState => ({
                                                                                                                                dataSlectContactArray: prevState.dataSlectContactArray.filter(item => item.contact_person_id_encode !== value)
                                                                                                                            }), () => console.log("Selected values:", this.state.dataSlectContactArray));
                                                                                                                        }
                                                                                                                    }}
                                                                                                                    value={item.contact_person_id_encode}
                                                                                                                />
                                                                ...
                                                                {companyType === 'new' &&
                                                                    <>
                                                                        <Row className="company-new company_radio_btn">
                                                                            <div className="create_customer_new" style={{ marginTop: "20px" }}>
                                                                                <CreateCustomerModal />
                                                                            </div>
                                                                        </Row>
                                                                    </>
                                                        ...
                                                        <div className="d-flex justify-content-between mt-3">
                                                            <Button color="primary" className={this.state.activeTab === 1 ? "previous disabled" : "previous"} onClick={() => { this.toggleTab(1) }}> Previous</Button>
                                                            <Button color="primary" type="submit" style={{ float: "right" }}>Next</Button>
                                                        </div>
                                                    </AvForm>
                                                </div>
                                            ...
                                            <TabPane tabId={3}>
                                                ...
                                                >
                                                                            ...
                                                                            <td>
                                                                                <h6>
                                                                                    <input
                                                                                        name={`document_type-${index}`}
                                                                                        type="file"
                                                                                        accept={this.getFileAcceptValue(index)}
                                                                                        onChange={(e) => {
                                                                                            const { rows } = this.state;
                                                                                            rows[index].document_type = e.target.value;
                                                                                            this.setState({ rows });
                                                                                    ...
                                                                                    />
                                                                                    <p className="allow_text_image">(Allowed file types: pdf, jpg, jpeg, png, docs, docx, xls, xlsx)</p>
                                                                                </h6>
                                                                            </td>
                                                                            <td>
                                                    ...
                                                    <div className="d-flex justify-content-between mt-3">
                                                        <Button color="primary" className={this.state.activeTab === 1 ? "previous disabled" : "previous"} onClick={() => { this.toggleTab(2); }}>Previous</Button>
                                                        <Button color="primary" type="submit" style={{ float: "right" }}>Next</Button>
                                                    </div>
                                                </AvForm>
                                            </TabPane>
                                            <TabPane tabId={4}>
                                                <div>
                                                    ...
                                                    >
                                                        ...
                                                        <div className="d-flex justify-content-between mt-3">
                                                            <Button color="primary" className={this.state.activeTab === 1 ? "previous disabled" : "previous"} onClick={() => { this.toggleTab(3); }}>Previous</Button>
                                                            <Button color="primary" type="submit" style={{ float: "right" }}>Next</Button>
                                                        </div>
                                                    </AvForm>
                                                </div>
                                            ...
                                            <TabPane tabId={5}>
                                                ...
                                                <div>
                                                    ...
                                                    >
                                                        ...
                                                        <Row>
                                                            ...
                                                            <Col lg="12">
                                                                <Card style={{ boxShadow: "5px 5px 10px 5px rgba(0, 0, 0, 0.1)" }}>
                                                                    <CardHeader>
                                                                        <h5 className="p-1 lead_overview" style={{ fontSize: "16px" }}>Lead Details</h5>
                                                                    </CardHeader>
                                                                    <CardBody>
                                                            ...
                                                            <Col lg="12">
                                                                <Card style={{ boxShadow: "5px 5px 10px 5px rgba(0, 0, 0, 0.1)" }}>
                                                                    <CardHeader>
                                                                        <h5 className="p-1 lead_overview" style={{ fontSize: "16px" }}>Company Details</h5>
                                                                    </CardHeader>
                                                                    <CardBody>
                                                            ...
                                                            <Col lg="12">
                                                                <Card style={{ boxShadow: "5px 5px 10px 5px rgba(0, 0, 0, 0.1)" }}>
                                                                    <CardHeader>
                                                                        <h5 className="p-1 lead_overview" style={{ fontSize: "16px" }}>Attach File</h5>
                                                                    </CardHeader>
                                                                    <CardBody>
                                                                        <Row className="mt-2  border-top">
                                                            ...
                                                            <Col lg="12">
                                                                <Card style={{ boxShadow: "5px 5px 10px 5px rgba(0, 0, 0, 0.1)" }}>
                                                                    <CardHeader>
                                                                        <h5 className="p-1 lead_overview" style={{ fontSize: "16px" }}>Activity Details</h5>
                                                                    </CardHeader>
                                                                    <CardBody>
                                                                        <Row className="mt-2  border-top">
                                                        ...
                                                        <div className="d-flex justify-content-between mt-3">
                                                            <Button color="primary" className={this.state.activeTab === 1 ? "previous disabled" : "previous"} onClick={() => { this.toggleTab(4); }}>Previous</Button>
                        ...
                        >
                            ...
                            <div className="text-center Contactperson" style={{ float: "right" }}>
                                <Row>
                                    ...
                                    <Col lg={4} >
                                        <Button color="secondary" className="cancle_contact_person"
