...
import { GET_LEAVE, GET_LEAVE_REQUEST_BY_ID, GET_LEAVE_TIME_SLOT, GET_LEAVE_WO_PAGINATE, GET_LOGO, GET_USERS_WITHOUT_ADMIN_, UPDATE_LEAVE_BALANCE_BY_LEAVE_ID, UPDATE_LEAVE_REQUEST, UPDATE_LEAVE_REQUEST_STATUS, UPDATE_USER, } from "../../globals";
...
// GET ALL TimeSlot
async GetAllTimeSlot(role_id) {
    var Token = localStorage.getItem("userToken");
    // var RoleId = localStorage.getItem("roleid");
    try {
        fetch(GET_LEAVE_TIME_SLOT + role_id, {
            method: "GET",
            headers: {
                Authorization: "Bearer " + Token,
                "Content-Type": "application/json",
                ...
                response.json().then((data) => {
                    if (data.result === true) {
                        if (data.data) {
                            ...
                            this.GetAllTimeSlot(data.data.role_id);
    ...
    handleSubmit(event, values) {
        ...
        // console.log('valuess---', values ,this.state.Timeslot , this.state.startTime, this.state.endTime)
       const ShortLeaveTimeslot = this.state.startTime + "-" + this.state.endTime
       console.log('valuess---', ShortLeaveTimeslot)
        ...
        try {
            fetch(UPDATE_LEAVE_REQUEST_STATUS + this.state.getById.id, {
                ...
                body: JSON.stringify({
                    ...
                    timeslot:values?.leave == "Short Leave" ? ShortLeaveTimeslot : values.timeslot,
    ...
    render() {
        console.log('data---', this.state.getById)
        return (
            <React.Fragment>
                                                    ...
                                                    <Col md="2">
                                                        ...
                                                        <Flatpickr
                                                            ...
                                                            onChange={(TIME) => {
                                                                ...
                                                                // console.log('onchnage--', startTime)
